...
const CCards = ({ image, subtitle, title, author, button }) => {
  ...
  return (
  <Card className="mt-6 min-w-sm max-w-96">
    ...
    <CardBody className="flex flex-col justify-between items-start">
      ...
      <div className="mt-10 w-full flex justify-center bg-orange-600 rounded-md hover:bg-[#1D5B79] active:bg-blue-800"> {/* Added flexbox properties */}
      <Link to={`/client/service-connect/${userId}`}>
        <Button className=" h-10 text-center p-2 ">
          {button}
        </Button>
      </Link>
    </div>
